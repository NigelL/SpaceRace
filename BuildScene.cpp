...
GameObject* cursor,*buildShip;
GameObject* buildHighlight;
Mesh* curShip;
int colorIndex;


UIObject* allColorOption = new UIObject[7];
Color allColor[6] = { Color(1,0,0),Color(0,1,0),Color(0,0,1),Color(1,1,0),Color(1,0,1),Color(0,0,0) };
std::vector<GameObject*> allBuildingBlocks;
...
BuildScene::BuildScene()
{
	...
	Mesh* buildMesh = MeshBuilder::GenerateCube("Cube", Color(0, 1, 0), 1, 1, 1);
	buildMesh->material.kAmbient.Set(0.4f, 0.4f, 0.4f);
	buildMesh->material.kDiffuse.Set(0.1f, 0.1f, 0.1f);
	buildMesh->material.kSpecular.Set(0.2f, 0.2f, 0.2f);
	buildMesh->material.kShininess = 1.0f;
	buildHighlight = new GameObject(buildMesh, Vector3(rand() % 5, 0, rand() % 5), 0, Vector3(0, 1, 0), Vector3(1.0f, 1.0f, 1.0f));
	meshList.push_back(buildHighlight);

	curShip = MeshBuilder::GenerateCube("Cube", Color(1, 1, 1), 1, 1, 1);
	...
	mainShip = new GameObject(curShip, Vector3(0, 0, 0), 0, Vector3(0, 1, 0), Vector3(1.0f, 1.0f, 1.0f));
	mainShip->GetTransform().SetBounds(Vector3(0.5f, 0.5f, 0.5f));

	meshList.push_back(mainShip);
	allBuildingBlocks.push_back(mainShip);
	...
	for (int i = 0; i < 6; i++) {
		allColorOption[i].curMesh = MeshBuilder::GenerateRealQuad("Test", allColor[i], 1, 1);
		allColorOption[i].size = 25;
		allColorOption[i].posX = -5;
		allColorOption[i].posY = 50 - (i * 5);
	}
	allColorOption[6].curMesh = MeshBuilder::GenerateRealQuad("Test", Color(1,1,1), 1, 1);
	allColorOption[6].curMesh->textureID = LoadTGA("Image//Cursor.tga");
	allColorOption[6].size = 25;
	allColorOption[6].posX = 25;
	allColorOption[6].posY = 5;

	allColorOption[0].curMesh->textureID = LoadTGA("Image//Red.tga");
	allColorOption[1].curMesh->textureID = LoadTGA("Image//Green.tga");
	allColorOption[2].curMesh->textureID = LoadTGA("Image//Blue.tga");
	allColorOption[3].curMesh->textureID = LoadTGA("Image//Yellow.tga");
	allColorOption[4].curMesh->textureID = LoadTGA("Image//Purple.tga");
	allColorOption[5].curMesh->textureID = LoadTGA("Image//Black.tga");
...
void BuildScene::RenderUpdateUI() {
	for (int i = 0; i < 7; i++) {
		RenderUI(allColorOption[i].curMesh, allColorOption[i].size, allColorOption[i].posX, allColorOption[i].posY);
	}
}
void BuildScene::ActivateBuildUI() {
...
//	glGetFloatv(GL_PROJECTION, modelView);
	...
	posX = ((2.0f *(float)mouseX) / 1920) - 1.0f;
	posY = 1.0f - ((2.0f *(float)mouseY) / 1440);
	posZ = 1.0;

	Vector3 ray_nds = Vector3(posX, posY, posZ);
	Vector3 rayNormal = ray_nds.Normalized();

	Vector3 ray_Clip = Vector3(ray_nds.x, ray_nds.y, -1.0f);
	Vector3 ray_eye = projectionStack.Top() * ray_Clip;
	ray_eye.z = -1.0f;
	Vector3 ray_wor = viewStack.Top().GetInverse() * ray_eye;

	ray_wor.Normalize();

	Vector3 mousePos = camera.up.Normalized() * posY + camera.right.Normalized() * posX;
	*/
	for (int i = 0; i < allBuildingBlocks.size(); i++) {
		if (Application::IsKeyPressed(VK_LBUTTON) && allBuildingBlocks[i]->collision.Intersect(camera.position, camera.view)) {
			buildShip = allBuildingBlocks[i];
		}
	}

	if (buildShip != nullptr) {
		float lowestVal = -1e9;
		Vector3 normalBuild = Vector3(0, 0, 0);

			//buildHighlight->SetPosition(Vector3(buildShip->GetPosition().x - camera.view.x, buildShip->GetPosition().y + camera.view.y, buildShip->GetPosition().z - camera.view.z));
			camera.view.Normalize();
			//std::cout << "Camera View : " << camera.view << std::endl;
			float xV = abs(camera.view.x);
			float yV = abs(camera.view.y);
			float zV = abs(camera.view.z);


			if (xV > lowestVal && xV != 0) {
				normalBuild = Vector3(-camera.view.x / abs(camera.view.x),0,0);
				lowestVal = xV;
			}
			if (yV > lowestVal && yV != 0) {
				normalBuild = Vector3(0,-camera.view.y / abs(camera.view.y ), 0);
				lowestVal = yV;
			}
			if (zV > lowestVal && zV != 0) {
				normalBuild = Vector3(0, 0,-camera.view.z / abs (camera.view.z ));
				lowestVal = zV;
			}
			
			if (normalBuild != Vector3(0, 0, 0)) {
				buildHighlight->SetPosition(buildShip->GetPosition() + normalBuild);
			}
			if (bounceTime <= 0.0f) {
				if (Application::IsKeyPressed(VK_RBUTTON)) {
					Mesh* newShipColorCube = MeshBuilder::GenerateCube("Cube", allColor[colorIndex], 1, 1, 1);
					GameObject* newShipCube = new GameObject(newShipColorCube, buildShip->GetPosition() + normalBuild, 0, Vector3(0, 1, 0), Vector3(1, 1, 1));
  
     				buildShip = newShipCube;
					allBuildingBlocks.push_back(newShipCube);
					meshList.push_back(newShipCube);
					bounceTime = 0.1f;
				}
			}
	...
	if (activatedUI) {
		cursor->SetPosition(Vector3(10000, 10000, 10000));
	}
	else {
		cursor->SetPosition(camera.position + camera.view);
...
void BuildScene::Update(double dt)
{
	...
	if (!activatedUI) {
		...
		camera.UpdateUI(dt, allColorOption[6]);
		
		for (int i = 0; i < 6; i++) {
			if (abs((allColorOption[i].posX - allColorOption[6].posX) + (allColorOption[i].posY - allColorOption[6].posY)) < 2) {
				colorIndex = i;
			}
	...
	//std::vector<Vector3> asada
	//parts push back ^
	//loop through then render them ()
	//std::erase when you collide ^
	
	UpdateMouseCursor();
	
	
	if (Application::IsKeyPressed(VK_TAB) && bounceTime <= 0) {
		ActivateBuildUI();
		bounceTime = 0.2f;
	}
	if (activatedUI) {
		for (int i = 0; i < 6; i++) {
			if (allColorOption[i].posX <= 5) {
				allColorOption[i].posX += dt * 10.0f;
			}
		}
	}
	else {
