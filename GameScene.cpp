...
GameScene::GameScene()
{
	...
	leftOption = MeshBuilder::GenerateMenu("left option", Color(1, 0, 0), 40);
	leftOption->textureID = LoadTGA("Image//leftOption.tga");
	rightOption = MeshBuilder::GenerateMenu("right option", Color(1, 0, 0), 40);
	rightOption->textureID = LoadTGA("Image//rightOption.tga");
...
void GameScene::renderWin()
{
	...
	if (winCondition1)
	{
		RenderUI(leftOption, 50, 27, 18.5, 20, 0, 0, 1, 0);
		RenderUI(Select, 20, 7, 70, UIy, 0, 0, 1, 0); // 11 29
	}

	if (winCondition2)
	{
		RenderUI(rightOption, 50, 27, 20, 20, 0, 0, 1, 0);
	}
	...
	if (resultTime <= 0)
	{
		...
		{
			GameSound::instance()->MenuMove->setDefaultVolume(1.0f);
			GameSound::instance()->engine->play2D(GameSound::instance()->MenuMove, false);
			resultTime = 0.2;
			UIy = 29;
		}
		...
		{
			GameSound::instance()->MenuSelect->setDefaultVolume(1.0f);
			GameSound::instance()->engine->play2D(GameSound::instance()->MenuSelect, false);
			resultTime = 0.2;
