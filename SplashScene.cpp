...
SplashScene::SplashScene()
{
	...
	Mesh* curShip = MeshBuilder::GenerateOBJ("ship 01", "OBJ//ship.obj");
	...
	mainShip = new GameObject(curShip, Vector3(2, 0, -2), 90, Vector3(0, 1, 0), Vector3(0.1, 0.1, 0.1));
	mainShip->SetBounds(Vector3(2.0f, 2.0f, 2.0f));
...
void SplashScene::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SplashScene::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y) {
...
void SplashScene::RenderUI(Mesh* mesh, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
	glDisable(GL_DEPTH_TEST);
...
inline void SplashScene::SetGameText(Mesh* curtext)
{
	gameText = curtext;
}
...
void SplashScene::Update(double dt)
{
	...
	camera.SetTarget(mainShip->GetPosition().x, mainShip->GetPosition().y + 1, mainShip->GetPosition().z);
...
void SplashScene::RenderMesh(GameObject* curType, bool enableLight)
{
...
void SplashScene::RenderMesh(Mesh* curMesh, bool enableLight) {
	Mtx44 modelView, modelView_inverse_transpose;
	MVP = projectionStack.Top() * viewStack.Top() * modelStack.Top();
	Mtx44 posPM = projectionStack.Top() * modelStack.Top();
	glUniformMatrix4fv(m_parameters[U_MVP], 1, GL_FALSE, &MVP.a[0]);
...
void SplashScene::RenderMesh(GEOMETRY_TYPE curType, bool enableLight)
{
	Mtx44 modelView, modelView_inverse_transpose;
	MVP = projectionStack.Top() * viewStack.Top() * modelStack.Top();
	Mtx44 posPM = projectionStack.Top() * modelStack.Top();
...
void SplashScene::Init2() {

}
void SplashScene::Update2(double dt) {

}
void SplashScene::Render2() {
